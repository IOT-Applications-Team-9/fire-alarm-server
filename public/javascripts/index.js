...
function register(){
    ...
    const deviceid = document.querySelector('.register-device-id').value
    const name = document.querySelector('.register-name').value
    const email = document.querySelector('.register-email').value
    const announce = document.querySelector('.register-announce')

    console.log(username, password1, password2, deviceid, email)

    if(!username || !password1 || !password2 || !deviceid || !name || !email){
        ...
        if(password1 != password2){
            ...
            xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            xhttp.send(`username=${username}&password=${password1}&deviceid=${deviceid}&name=${name}&email=${email}`)
        }
    }
}