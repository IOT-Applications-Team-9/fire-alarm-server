...
module.exports = function(app, mqttClient){
    ...
    app.get('/user/:uid/home', function(req, res){
        ...
        conn.query(`select * from user where id = ?`, [uid], function(err, results){
            ...
            queryList.push(`select * from environment_state where deviceid = ${deviceid} and thoigian between '${year}-${month}-${day}' and '${year}-${month}-${day} 23:59:59' order by thoigian desc limit 1`) 

            // lấy giá trị trung bình trong ngày
            
            queryList.push(`select avg(temperature) as atemp, avg(humidity) as ahumi, avg(fire) as afire, avg(gas) as agas from environment_state where deviceid = ${deviceid} and thoigian between '${year}-${month}-${day}' and '${year}-${month}-${day} 23:59:59'`) 

            // Lấy trạng thái hiện tại của hệ thống
            queryList.push(`select * from system_state where deviceid = ${deviceid}`)
            conn.query(queryList.join(';'), function(err, results){
                ...
                // Thông số hiện tại
                var curState = {};
                console.log(results[0][0].temperature)
    ...
    app.get('/user/:uid/updateData', function(req, res){
        ...
        // lấy danh sách dữ liệu mới nhất ngày hôm nay
        queryList.push(`select * from environment_state where deviceid = ${deviceid} and thoigian between '${year}-${month}-${day}' and '${year}-${month}-${day} 23:59:59' order by thoigian desc limit 10`) 
        ...
        conn.query(queryList.join(';'), function(err, results){
            ...
            if(curWarning != 'Không rõ' && curWarning != 'An toàn'){
                ...
                const timeStampInMinute = 1 // 15p cập nhật 1 lần
                ...
                if(curTime - lastAnnounce > timeStampInMillis){
                    ...
                    mailer.sendEMail(email, "FIRE ALARM SYSTEM: WARNING!!!", str, function(err, info){
                        if(err) throw err
